...
//            By.xpath("//*[@placeholder=\"Date from\"]");
    ...
    By dateToList= By.xpath("//*[@placeholder=\"Date to\"]");
...
//        System.out.println(pro.getProperty("https://demo1.leotheme.com/ap_booking_demo/en/home-2.html"));
    ...
    public void validateSelectDateFrom(){
        ...
        driver.findElement(dateFromIcon).click();
        selectDateFrom();
        Assert.assertTrue(driver.findElement(dateFromList).getText().contains("22"));
    ...
    public void validateSelectDateTo() {
        ...
        driver.findElement(dateToIcon).click();
        selectDateTo();
        Assert.assertTrue(driver.findElement(dateToList).getText().contains("24"));
    }
    ...
    public void iSelectChildernsOption(){
        String childValue = "0";
        //Validate default value for adults filter
        Assert.assertEquals(childValue, driver.findElement(childplaceholder).getText());
    ...
    public void validateDateToDefaultValue(){
        ...
        Assert.assertEquals(driver.findElement(dateFromList).getText(),formattedDate);
    }

    public void validateDateFromDefaultValue(){
        LocalDate yesterday = LocalDate.now().minusDays(1);
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        String dateTo = yesterday.format(formatter);
        System.out.println(driver.findElement(dateFromList).getText());
        Assert.assertEquals(driver.findElement(dateFromList).getText(),dateTo);
    }
    ...
    public void selectDateFrom() {
        ...
        for (int i=1; i< dateRows.size();i++){
            for (int colIndex =1;i<dateCol.size();colIndex++){
    ...
    public void selectDateTo() {
        ...
        for (int i=1; i< dateRows.size();i++){
            for (int colIndex =1;i<dateCol.size();colIndex++){
                if (findElementElementByDynamicLocatorAndIndex(i,colIndex).getText().equals("26")){
                    findElementElementByDynamicLocatorAndIndex(i,colIndex).click();
                }
            }
        }
